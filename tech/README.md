...
- Does not cover H2 (centralised information hub) or H4 (role-specific content) — both were flagged as too broad for a Week 3 code slice and are represented in Figma only with static sample data.
- The contextual help panel currently only shows "who to ask" — it does not yet include the AI-assistant half of H3 (source-cited answers). That part remains a stretch goal for Week 4 if time allows.
